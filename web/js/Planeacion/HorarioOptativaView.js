...
function inicio(contex) {
    ...
    $('#tableHorario tbody').on('click', 'td .modificar', function (e) {
        ...
        var idHorarioOptativa = $(this).data("id");
        var idProfesor = $(this).data("profesor");
        var semestre = $(this).data("semestre");
        var idPlanEstudios = $(this).data("idplanestudios");
        var nombreOptativa = $(this).data("nombreoptativa");
                        
        $.getJSON(context + '/consultarProfesorHorarioOptativa', {
            idHorarioOptativa: idHorarioOptativa, 
            semestre: semestre,
            ...
            $('#idOptativasM').find('option').remove();
            var selecciones = $('<option value="">Seleccione...</option>');
            $('#idOptativasM').append(selecciones);
            $.each(data.optativaHorario, function(posicion, optativa) {

                var opcion = $('<option value="' + optativa.idOptativas + '">' + optativa.nombreOptativa + '</option>')
                if(nombreOptativa==optativa.idOptativas){
                    opcion.attr("selected",true);
                }
                $('#idOptativasM').append(opcion);
                });
        });
                        

        //Subo los valores a los input's
        $("#idHorarioOptativaM").val($(this).data("id"));
        $("#nombreMateriaM").val($(this).data("nombremateria"));
        $("#nombreOptativaM").val($(this).data("nombreoptativa"));
        ...
        function(e) {
            ...
            alertify.confirm("¿Estás seguro de eliminar el horario?", function(e) {
                if (e) {
                    ...
                    $('#idHorarioOptativaM').val(idHO);
                    $('#form2').attr('action', context + "/eliminarHorarioOptativa");
                    $('#form2').submit();
                } else {
